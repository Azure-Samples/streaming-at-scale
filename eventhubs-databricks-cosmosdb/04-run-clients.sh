...
locust_output=$(az group deployment create -g $RESOURCE_GROUP --template-file arm/locust.arm.json --parameters eventHubNamespace=$EVENTHUB_NAMESPACE eventHubName=$EVENTHUB_NAME eventHubKey=$EVENTHUB_KEY storageAccountName=$AZURE_STORAGE_ACCOUNT fileShareName=locust numberOfInstances=$TEST_CLIENTS)
...
curl $LOCUST_MONITOR/swarm -X POST -F "locust_count=$USER_COUNT" -F "hatch_rate=$HATCH_RATE"
