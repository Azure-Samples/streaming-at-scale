...
usage() { 
    ...
    echo "-t: test 1,5,8 thousands msgs/sec. Default=$TESTTYPE"
...
echo "***** [D] Setting up DATABASE"
    ...
    export TSI_ENVIRONMENT=$PREFIX"tsi" 

    RUN=`echo $STEPS | grep D -o || true`
    if [ ! -z "$RUN" ]; then
        source ../components/azure-timeseriesinsights/create-timeseriesinsights.sh
    fi
echo
