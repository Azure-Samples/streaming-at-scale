...
object StreamingJob {
      ...
      .apply((key, window, input, out: Collector[EventStats]) => {
        ...
        if (it.hasNext) {
          ...
          if (it.hasNext) {
            ...
            val eventNumberDeltaCounts = e2.get("eventNumber").longValue - e1.get("eventNumber").longValue
            ...
            out.collect(EventStats(Map(eventNumberDeltaCounts -> 1L), eventAge))
          }
        }
      })
  ...
  case class EventStats(eventNumberDeltaCounts: Map[Long, Long], sumOfLatencies: Long)
  ...
  class ComputeEventRateProcessFunction
    ...
    override def process(context: Context, elements: Iterable[EventStats], out: Collector[String]): Unit = {
      ...
      val anomalousEventNumberDeltaCounts = singleItem.eventNumberDeltaCounts.filterKeys(_ != 1)
