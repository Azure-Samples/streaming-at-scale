...
if [ "$INGESTION_PLATFORM" == "eventhubskafka" ]; then
  source ../components/azure-event-hubs/get-eventhubs-kafka-brokers.sh "$EVENTHUB_NAMESPACE_OUT" "Listen"
fi

  KAFKA_OUT_LISTEN_BROKERS=$KAFKA_BROKERS
  KAFKA_OUT_LISTEN_SASL_MECHANISM=$KAFKA_SASL_MECHANISM
  KAFKA_OUT_LISTEN_SECURITY_PROTOCOL=$KAFKA_SECURITY_PROTOCOL
  KAFKA_OUT_LISTEN_JAAS_CONFIG=$KAFKA_SASL_JAAS_CONFIG
