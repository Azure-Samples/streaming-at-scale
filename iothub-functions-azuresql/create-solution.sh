...
# 1000 messages/sec
if [ "$TESTTYPE" == "1" ]; then
    export IOTHUB_SKU=S1
    export IOTHUB_UNITS=1
...
echo ". IoT Hub         => SKU: $IOTHUB_SKU, Units: $IOTHUB_UNITS, Partitions: $IOTHUB_PARTITIONS"
...
echo "***** [P] Setting up PROCESSING"
    ...
    export PROC_PACKAGE_FOLDER=../eventhubs-functions-azuresql
...
echo "***** [T] Starting up TEST clients"
    ...
    if [ ! -z "$RUN" ]; then
        source ../simulator/run-generator-iothub.sh
    fi
echo
