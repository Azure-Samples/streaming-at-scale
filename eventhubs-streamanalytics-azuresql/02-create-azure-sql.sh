...
az storage container create --name database \
    -o tsv >> log.txt

echo "updloading bacpac"
az storage blob upload --container-name database --name streaming.bacpac --file streaming.bacpac \
    -o tsv >> log.txt

echo "retrieving blob url"
BLOB_URL="$(az storage blob url --container-name database --name streaming.bacpac -o tsv)"

echo "retrieving container sas"
EXPIRE_ON=$(date -u -d "30 minutes" '+%Y-%m-%dT%H:%MZ')
BLOB_SAS="?$(az storage container generate-sas --name database --permissions lr --https-only --expiry $EXPIRE_ON -o tsv)"
